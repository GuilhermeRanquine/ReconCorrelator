    ...
    if (programUrl && !rawPolicyText) {
      try {
        ...
        if (response.ok) {
          ...
          // Extract text snippet to avoid huge payloads
        ...
        pageContentToAnalyze = `URL do programa informada pelo pesquisador: ${programUrl}. Identifique os ativos e o escopo associados a esta organização.`;
      }
    }

    const ai = getAiClient();

    const systemInstruction = `Você é um Analista Sênior de Reconhecimento e Triage de Bug Bounty (Squad ALPHA/Red Team).
...
5. Regras de engajamento, Safe Harbor (se tem proteção jurídica expressa) e vulnerabilidades proibidas (DDoS, Self-XSS, Rate-Limit sem impacto, SPF/DMARC puro).
6. Headers customizados obrigatórios (ex: 'X-Bug-Bounty: hacker_username').
7. Faixas de Bounty em USD por severidade (se disponível).
8. Arquitetura predominante do alvo ('spa_web', 'microservices_api', 'cloud_native', 'hybrid', 'mobile_backend').`;

    const userPrompt = `
Analise o programa de Bug Bounty abaixo:
...
`;
        ...
        responseSchema: {
          ...
          properties: {
            ...
            prohibitedVulns: {
              type: Type.ARRAY,
              items: { type: Type.STRING },
              description: 'Tipos de vulnerabilidade expressamente fora de escopo'
            },
            requiredHeaders: {
              type: Type.ARRAY,
              items: {
    ...
    const policy: BugBountyPolicy = {
      ...
      policySummary: parsedJson.policySummary || 'Respeite as taxas de requisição e não teste DoS/DDoS.',
      safeHarbor: parsedJson.safeHarbor ?? true,
      prohibitedVulns: parsedJson.prohibitedVulns || ['DDoS', 'Social Engineering', 'Physical Attacks', 'Self-XSS', 'Missing SPF/DMARC records'],
      requiredHeaders: parsedJson.requiredHeaders || [{ key: 'X-BugBounty-Hacker', value: 'recon-squad-34', description: 'Header de identificação do pesquisador' }],
